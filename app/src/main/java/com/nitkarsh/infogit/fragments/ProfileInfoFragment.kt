...
//        returns if argument dont contains login which is a necessity
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        tvFollowers.setOnClickListener {
            (activity as MainActivity).loadFollowersFragment(login!!)
        }

//      Observer observing the data change then loading the data into list to set it up in recyclerview.
        userDetailViewModel.usersResponse.observe(this, Observer {
            it?.let {
                ...
                if (!it.createdAt.isNullOrEmpty()) {
                    listData.add(
                        Store(
                            getString(R.string.account_creation_date),
                            Utils.convertDateViaFormatTZ(it.createdAt!!)
                        )
                    )
                }
